...
Direct-to-customer Goal Planner. React UI + FastAPI BFF that maps a session into
**FM** (People Like You / Need Profiler / Need Calculator), **HU** (HappiU), and
**SV** (Scenario Visualizer). GP does not run Monte Carlo itself.

This repo is GP only. Point `FM_UPSTREAM`, `HU_UPSTREAM`, and `SV_UPSTREAM` at
the engines you have been given. See [docs/Standalone-third-party.md](docs/Standalone-third-party.md).
...
| `POST` | `/v1/predict` | FM pipeline (503 if People Like You is down) |
