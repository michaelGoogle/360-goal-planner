...
GP BFF  (FastAPI)
        ...
        ├── POST /v1/predict         → FM People Like You
        │                            → FM Need Profiler
        │                            → FM Need Calculator
        │                            (503 if People Like You is down)
        ├── POST /v1/score           → HU POST /v1/happi-u
        └── POST /v1/project         → SV POST /api/v2/scenario-visualizer
```

GP is a **BFF + customer UI**, not an engine. Mapping lives in
`hu_payload.py` / `sv_payload.py`. Prompts instruct the model to use only the
JSON context the UI already displayed.
...
- **Compose:** [`Dockerfile`](../Dockerfile) in this repo bakes `frontend/dist`
  and serves it from the API process when that folder exists.
...
2. Call FM **people-like-you** (public, or `/v1/me/onboarding/...` when
   `Authorization` is present and `persist` is true).
...
5. Need Profiler (`topN=5`) then Need Calculator (`onlyEmpty=true`).
6. Return `{ success, session, notes }`. Notes list which upstreams were skipped.
